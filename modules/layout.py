...
def smooth_xpos(node, n2coords, collapsed={}):
    if (not node.istip) and (node.id not in collapsed):
...
def calc_node_positions(node, width, height,
    ...
    depth_length_preorder_traversal(node, n2coords=n2coords,
                                    collapsed=collapsed)
    leaves = node.leaves(collapsed=collapsed)
    nleaves = len(leaves)
    ...
    for n in node.iternodes(phylo.POSTORDER, collapsed=collapsed):
        c = n2coords[n]
        if (not n.istip) and (n.id not in collapsed) and n.children:
            children = n.children
